...
// The marketing site's installed PWA subscribes cross-origin (gunnerthelab.com → app.gunnerthelab.com).
push.use(
  '*',
  cors({
    origin: ['https://gunnerthelab.com', 'https://www.gunnerthelab.com'],
...
push.post('/subscribe', async (c) => {
  if (c.req.header('x-requested-with') !== 'storyreader') return c.json({ error: 'missing_csrf_header' }, 403);
