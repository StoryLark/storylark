...
/** Next chapter in the book; for one-chapter (gunner story) books, the next book's story. */
...
/** True when this brand's UI treats every book as a standalone story (flat library). */
export function isStoryBrand(): boolean {
  return BRAND.id === 'gunner';
}
