...
function SyncSection(): JSX.Element {
  ...
  const autoDownloadLabel =
    BRAND.id === 'gunner' ? `Auto-download new ${NOUNS.unitPlural} (incl. audio)` : `Keep the whole ${NOUNS.collection} downloaded (incl. audio)`;
  return (
    <section class="settings-section">
      <h2>Library & sync</h2>
      <label class="settings-row">
      ...
      <label class="settings-row">
        ...
        <input
          ...
          onChange={(e) => {
            const on = (e.target as HTMLInputElement).checked;
            void (async () => {
              if (on && BRAND.id === 'gunner') await setAutoDownloadBaseline();
