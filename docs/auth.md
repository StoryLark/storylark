...
- **Google** — server-side OAuth code flow; the sign-in button isn't shown
  yet (tracked in ADO), but the route works.
...
(tracked in ADO); the route itself works today if called directly.
...
that every mutating request must carry `X-Requested-With: storyreader`, which a
cross-site form can't add. JSON-only bodies close the rest.

## Setup still required per brand

- `RESEND_API_KEY` secret (holdfastpress.com domain already verified in Resend;
  verify gunnerthelab.com before Gunner launch or send from holdfastpress.com).
...
- Passkeys need no new secret or external account: `rpID`/`rpName`/`origin` all come
  ...
  storyreader-<brand> --env <brand> --remote`) before first use.
